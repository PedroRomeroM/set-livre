...
- FEAT-003 em implementação no [PR #4](https://github.com/PedroRomeroM/set-livre/pull/4), com perfil PF/PJ, `/conta`, segurança e preferência visual integrados. O PR foi marcado ready no HEAD `9531815`; a revisão Codex publicada às `03:45Z` abriu dois hardenings P2 adicionais, sobre logout offline/obsoleto e timeout da projeção de preferência no login. As correções foram congeladas localmente no commit `e7cc8378c1c0a721f64ad3fc21dd61dca9086ef7`, e a release canônica desse snapshot já foi gerada e auditada; publicação/push, respostas e resolução dos dois P2, nova revisão e merge continuam pendentes. As afirmações anteriores de zero thread e “somente merge pendente” permanecem apenas como histórico da revisão do HEAD `9c23ef3`;
- aplicações pública e backoffice separadas;
- Node/npm e dependências fixados em lockfile;
- Supabase local via Docker com 12 migrations append-only e head `20260811000500`; no snapshot P2 corrente, reset, geração e 293/293 asserts pgTAP passaram, distribuídos em 158 + 78 + 57, sem resíduo depois da execução;
- o snapshot P2 corrente passou em 578/578 unitários de 60 arquivos e em uma matriz Playwright/axe integral de 91/91 em 3,9 minutos — 32/32 da FEAT-003, cobrindo sem alterar contagens os IDs `SL-F003-E2E-001` a `009`. O ID 004 passou em 3/3 projeções: logout stale recebeu `409`, a sessão e o perfil de B permaneceram intactos e houve zero `pageerror` ou erro React; o ID 009 passou em 4/4, com falha offline imediata, exatamente uma request e nenhum POST tardio após reconexão;
- a auditoria browser corrente terminou com zero resultado inesperado, flake, skip, erro ou attachment e não encontrou sentinelas, tokens, cookies Auth nem documentos crus. Os 62 e-mails QA sintéticos únicos apareceram em 114 ocorrências exclusivamente nos títulos automáticos allowlisted dos steps: `Fill` 84, `Visible` 18, `Count` 4 e `Type` 8. Cleanup de banco, Mailpit, portas e processos terminou sem resíduos;
