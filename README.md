...
- O snapshot Windows atual passou por `npm ci`, format, lint, typecheck, 991 testes unitários (27 skips condicionais de plataforma), docs:check com 34 features/200 cenários/23 ADRs, audit sem vulnerabilidades, Knip e diff-check. A matriz Playwright integral passou em 114/114, 17 specs e 16 projetos, sem retry, e os builds standalone web/backoffice terminaram com 26 + 4 rotas. Essa evidência é local e não antecipa o CI hospedado, a release Linux x86_64 nem o deploy.
- Os workflows de exemplo de outro projeto foram substituídos por CI/CD próprios do Set Livre. O deploy permanece fail-closed (`PRD_DEPLOY_ENABLED=false`) até que configuração, PR, reviews, Supabase São Paulo, VM Oracle e smoke real estejam comprovados.
