...
Para uma visão direta das tecnologias verificadas no repositório, consulte [`docs/technology-stack.md`](docs/technology-stack.md). Toda mudança destinada a `main` obedece ao [ciclo completo obrigatório de review e deploy](docs/review-deploy-cycle.md): revisão Codex do SHA final, espera mínima real de 60 minutos em cada rodada, correção/resolução de todos os findings e monitoramento pós-merge até o health público.
...
### Ambiente Windows e entrega cloud controlada

- O desenvolvimento local foi migrado para Windows nativo com Node 24.18.0, npm 11.19.0 e Docker Desktop. Processos filhos usam Windows Job Objects; filesystem, arquivos de ambiente e árvore Docker/Supabase são verificados por contratos específicos em vez de emular o comportamento POSIX.
- `main` já contém as FEAT-002, FEAT-003 e FEAT-004 pelos PRs #2, #4 e #6; são três das 34 features. O relato detalhado do quinto P2 abaixo é uma fotografia pré-merge preservada para auditoria, não o status atual da feature.
- A migration append-only `20260819000100_supabase_rls_event_trigger_acl_hardening.sql` atualiza o head local sem alterar migrations aplicadas. A cadeia canônica `supabase:reset` → `supabase:generate` → `test:db` passou em 361/361 asserts, com 16 migrations e artefatos gerados sincronizados.
- O snapshot Windows atual passou por `npm ci`, format, lint, typecheck, 986 testes unitários (33 skips condicionais de plataforma), docs:check com 34 features/200 cenários/23 ADRs, audit sem vulnerabilidades, Knip e diff-check. A matriz Playwright integral passou em 114/114, 17 specs e 16 projetos, sem retry, e os builds standalone web/backoffice terminaram com 26 + 4 rotas. Essa evidência é local e não antecipa o CI hospedado, a release Linux x86_64 nem o deploy.
- Os workflows de exemplo de outro projeto foram substituídos por CI/CD próprios do Set Livre. O projeto Supabase de São Paulo já está saudável e publicamente vinculado, mas permanece vazio; o deploy continua fail-closed (`PRD_DEPLOY_ENABLED=false`) até configuração privada, PR, reviews, VM Oracle, migrations e smoke real.
- O alvo de runtime é exclusivamente Oracle `VM.Standard.E2.1.Micro` Always Free, Ubuntu 24.04 x86_64, Nginx, systemd e releases imutáveis por SHA. Não há fallback para outra shape, região ou recurso pago.
- O ADR-022 torna obrigatório corrigir incompatibilidades na fonte canônica: não são aceitos workaround, fallback silencioso, edição manual de artefato gerado ou exceção usada para transformar gate inconclusivo em sucesso.

### Fotografia histórica do quinto P2 da FEAT-004

- Naquele snapshot pré-merge, a FEAT-004 ainda estava **Em implementação**. O review `PRR_kwDOTyzZrs8AAAABJsAUGQ`/REST `4945089561`, submetido em `2026-08-16T01:07:18Z` sobre `0decf00`, abriu a thread `PRRT_kwDOTyzZrs6Zj15h`, comentário `PRRC_kwDOTyzZrs7h8LaV`/REST `3790648981`, em `owner-recipient-panel.tsx` linha 489. Na captura, a thread estava atual, não resolvida e não desatualizada: o documento `local_fixture` podia continuar acompanhado do formulário de aceite em um runtime no qual a ativação seria recusada.
...
- O commit funcional `2045d1a00c15889007b3c5c04c08d0467fc3d9b3` foi processado exatamente uma vez pelo gerador canônico: exit `0` em 21,26 s. O primeiro smoke P5 foi o embutido e ficou verde antes da publicação local do archive. `.artifacts/set-livre-2045d1a...tar.gz` possui 24.896.963 bytes, modo `0600` e SHA-256 `282f9d173eebf99ba63466d81f4aa4b9061e7d73668c267fb0a25e9e86043b92`; sidecar 124 bytes/`8955c004a68401dfd27190d26ac1e92157a635fbf191977c2d7408e4c95f1eb0`, manifesto 681.762 bytes/`d8b698ecef6b6c52f4961e8783ef2c1e68b5ab00239de4de9206cb9f2f2d2026` e log 2.097 bytes/`505a5fd915bacd59d3deea9c16c615cee82a9026ba17ac66ac6e4475a4c8d40e`. São 2.871 artefatos e 3.455 membros no tar; ambos os `BUILD_ID` equivalem ao commit. Duas auditorias `NO-BLOCKER` fecharam paridade, smoke, privacidade e cleanup sem mismatch ou resíduo. A release era somente local Linux x64 Node 24.18/npm 11.19, ignorada e não publicada; naquele snapshot pré-ADR-021, ARM64/Oracle/PEND-003 e remoto ainda constavam como pendentes. O requisito atual foi substituído pelo smoke Linux x86_64 na E2 Micro. Próxima ação daquela fotografia: commit dos docs, push, atualizar body, responder/resolver P5, novo review, aguardar 60 minutos, captura final, ready e merge.
...
- a release `440c81f6...` foi canônica somente para o segundo P2: possui 14 migrations/head `20260812000200` e agora é histórica, assim como `c115dcd...`, `79376b62...` e a release do terceiro P2; nenhuma contém a capability atual. Naquele snapshot pré-ADR-021, PEND-003/smoke ARM64 ainda eram obrigatórios; hoje o gate equivalente é Linux x86_64 na E2 Micro. A feature permanece **Em implementação**;
...
O ADR-019 substitui somente a suspensão de infraestrutura do ADR-018 e autoriza uma entrega cloud controlada. CI/CD, Supabase Cloud, Oracle, domínio e TLS continuam bloqueados por gates fail-closed enquanto não houver evidência real; providers de pagamentos e demais integrações externas permanecem suspensos.
...
- GitHub Actions;
- Oracle Cloud `VM.Standard.E2.1.Micro` Always Free x86_64;
