...
### Terceiro P2 da FEAT-004

- A FEAT-004 continua **Em implementação**. O review abriu `PRRT_kwDOTyzZrs6ZhR_d`/REST `3789698299`: a auditoria persistia a chave idempotente do browser como request ID. A correção local separa `requestId` de correlação e `idempotencyKey` de replay em toda a cadeia, sem alterar os sete IDs Playwright.
- A árvore atual possui 15 migrations append-only e head `20260815000100`. Um único reset, geração e `test:db` passou em 358/358 (`158 + 78 + 57 + 65`); readiness aceita o head atual e recusa `20260812000200`, os overloads/grants e gerados estão sincronizados e as tabelas verificadas terminam em zero.
- O fechamento precommit atual em Node 24 passou `npm ci` com 447 pacotes e auditoria em zero, format, lint, typecheck, 42/42 unitários focados, 12/12 guardas estáticos de privacidade, 718/718 unitários integrais em 74 arquivos, docs:check em 34 features/200 cenários/18 ADRs, `npm audit` em zero, Knip e diff-check.
- A rodada browser focada P3 anterior permanece verde em 23/23. Uma primeira integral do snapshot sem a correção do oráculo terminou em 79 passados, uma falha e 34 não executados no `FOUNDATION-E2E-008` WebKit; ela não foi repetida. Após separar as páginas do teste, o crítico passou em 3/3 e a integral corrigida passou exatamente em 114/114, 17 specs e 16 projetos, preservando 23/23 da FEAT-004, com privacidade e cleanup verdes.
- Um único build atual terminou com exit `0`, 26 rotas web e quatro do backoffice, sem warnings. Um único smoke customizado terminou com exit `0` em 2,4 segundos, três probes guest `401` com UUID, dois redirects exatos, 14/11 nonces e banco/Mailpit `0 → 0`, com privacidade e cleanup verdes. A release canônica local final do commit funcional `2a86acc4dc3a005213d5f22384084e3aba0160be` foi gerada e auditada: o archive possui 24.903.588 bytes, SHA-256 `0e0c07f41d4a44f0673ce7a5013084942100e8baab1ba72ee6aeea6496be1566` e 2.870 artefatos, com 15 migrations/head `20260815000100`; duas auditorias independentes terminaram `NO-BLOCKER`. Na captura remota de `2026-08-15T19:38:32Z`, o push `3dd11cb → dda95b3` publicou o funcional e a documentação da release até `dda95b3b9108930489a3b10275ef41c2f203ae24`; a resposta `PRRC_kwDOTyzZrs7h6HnW`/REST `3790109142` foi criada por `PedroRomeroM`, e a thread ficou resolvida, atual e sem pendências. O commit/push deste novo registro, eventual atualização do body do PR, novo `@codex review`, espera mínima de 60 minutos, captura final única, ready e merge permanecem pendentes; a prova continua local x64 e não substitui o smoke ARM64.

### Fotografia histórica anterior ao terceiro P2

Os registros abaixo pertencem ao snapshot fechado do segundo P2. Seus artefatos 23/23, 114/114, build/smoke e release `440c81f6...` permanecem históricos e não substituem as novas execuções descritas acima nem validam o head `20260815000100`.
...
- a release `440c81f6...` foi canônica somente para o segundo P2: possui 14 migrations/head `20260812000200` e agora é histórica, assim como `c115dcd...` e `79376b62...`; nenhuma contém a correção atual. A feature permanece **Em implementação**, e PEND-003/smoke ARM64 continuam obrigatórios para produção;
