...
### Estado vivo do terceiro P2 da FEAT-004

O review abriu `PRRT_kwDOTyzZrs6ZhR_d`/REST `3789698299` porque `audit.events.request_id` recebia a chave idempotente do browser. A migration append-only `20260815000100_owner_audit_request_correlation.sql` cria `idempotency_key NOT NULL`, move a unicidade para a chave lógica e reserva `request_id` para correlação. O repositório possui 15 migrations; 42/42 unitários focados e 358/358 pgTAP (`158 + 78 + 57 + 65`) passaram, com readiness, gerados e cleanup no head atual. O fechamento estático precommit passou `npm ci` com 447 pacotes/auditoria zero, format, lint, typecheck, 12/12 guardas de privacidade, 718/718 unitários em 74 arquivos, docs:check 34/200/18, `npm audit` zero, Knip e diff-check.
...
Um único build atual passou com exit `0`, 26 rotas web, quatro do backoffice e zero warning; log SHA-256 `8677b868a632e0891499c8450e5c926ddefcde7e27c5d31f9adcb55e27bbfaa2`. Um único smoke customizado passou com exit `0` em 2,4 segundos: três probes guest `401` com UUID, dois redirects exatos, 14/11 nonces, banco/Mailpit `0 → 0`, privacidade e cleanup verdes; hashes SHA-256: stdout `399d3b41dd9d161bdd86288c53e5bf821279285eb4772740c9ff5169845e5abd`, server log `e3c376cdc9403d2739ea8f127244fef193ea0ad4689694fec5c8a097d5ee025b` e resumo `25262fb6efbf93a0a654a16171bc4f6998000ef0078b9d91e40a06beefe79450`.
...
No segundo P2 histórico, todos os gates Node 24 ficaram verdes: 718/718 unitários, banco 355/355 e execução focada 23/23. Essa fotografia não valida o head atual.
...
A release `440c81f6...`, assim como `c115dcd...` e `79376b62...`, é histórica diante do terceiro P2. A FEAT-004 segue **Em implementação**.

As quatro threads estão respondidas e resolvidas; o terceiro P2 `PRRT_kwDOTyzZrs6ZhR_d` já possui gates locais, banco, browser, build, smoke, release canônica local final, publicação até `dda95b3...` e resposta/resolução verificadas. Ainda exige commit/push deste registro, atualização do body do PR se necessária, novo review com espera mínima de 60 minutos e captura final única. Não há claim de ready ou merge.
