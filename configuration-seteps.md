...
- [x] Projeto Supabase Cloud `Set Livre Production`, ref pública `oirvvnojgkzdppkdvhej`, criado em `sa-east-1` (São Paulo), na organização `PedroRomeroM's Org`, e comprovado `ACTIVE_HEALTHY`. O projeto canadense anterior foi apagado sem migrations.
- [x] Workstation Windows 11 nativa, sem WSL, com Node `24.18.0`, npm `11.19.0`, Git `2.55.0`, GitHub CLI `2.97.0`, OCI CLI `3.90.1`, `psql` `18.6`, PowerShell oficial `7.6.5`, Docker Desktop `4.87.0`, actionlint `1.7.12` e ShellCheck `0.11.0` instalados. O PowerShell operacional é exclusivamente o MSI oficial x64 em `C:\\Program Files\\PowerShell\\7\\pwsh.exe`; o MSIX redundante foi removido depois da validação do binário, hash e assinatura.
...
- [x] A tenancy admite duas E2 Micro Always Free e outra instância já existe fora do Set Livre. A posição usada pelo diagnóstico foi liberada. Em `2026-08-23`, sessão, inventário, ACLs, estado privado v1→v2 e sondas não persistentes repetidas foram revalidados; todas encerraram fail-closed com `OUT_OF_HOST_CAPACITY`, sem autorização implícita para shape pago, outra região ou outro projeto.
- [x] SVM/virtualização habilitada no firmware; após o reboot de `2026-08-17 22:05:28`, `VirtualizationFirmwareEnabled=True`.
- [x] Docker Desktop `4.87.0` saudável no backend Hyper-V/Linux, contexto `desktop-linux`, endpoint `npipe:////./pipe/dockerDesktopLinuxEngine`, engine `29.7.2` e `OSType=linux`.
- [x] GitHub CLI está autenticado. O perfil OCI `SET_LIVRE` foi renovado e validado remotamente em `2026-08-23`; a sessão é temporária e precisa ser renovada quando expirar. Os MCPs Supabase permanecem separados por projeto; qualquer renovação OAuth precisa concluir no navegador e ser comprovada por leitura do projeto correto.
...
- [x] O projeto canadense anterior permaneceu sem migrations e foi apagado. Seus 2 WARN históricos de `rls_auto_enable` não foram transportados como prova do projeto novo; a correção canônica continua na migration append-only local `20260819000100`, ainda não aplicada no Cloud.
- [x] A cotação oficial de US$ 10/mês foi exibida e aprovada explicitamente; `Set Livre Production` foi criado em `sa-east-1` e sua identidade foi validada por MCP, código e repository variables.
- [x] O projeto novo iniciou com zero migrations, zero tabelas próprias e zero branches. O Security Advisor retornou zero findings; o Performance Advisor retornou somente o INFO `auth_db_connections_absolute`.
- [x] Supabase Branching está fora desta fase: desenvolvimento/testes destrutivos usam somente Supabase local e produção recebe somente merges protegidos em `main`.
...
- [x] Projeto produtivo criado com nome, ref, região e organização exatos; o projeto canadense não foi reutilizado.
- [ ] No painel da organização, confirme owner adicional/contatos de recuperação e MFA organizacional antes do go-live.
- [x] A senha administrativa do banco foi rotacionada pela Management API com 256 bits aleatórios e guardada somente no Gerenciador de Credenciais do Windows, target `Set Livre:Supabase production database password`; nenhum valor foi impresso ou versionado. Depois que o configurador criar o diretório privado da VM, copie-a em sessão protegida para `/etc/setlivre-deployer/credentials/supabase-db-password`, arquivo físico `root:root` modo `0600` entregue ao agente por `LoadCredential`.
- [x] Project ref, URL e publishable key moderna foram lidas do projeto novo e gravadas nas repository variables com round-trip exato; `PRD_DEPLOY_ENABLED=false` foi preservado.
- [ ] Depois de provisionar a VM, confira os mesmos valores em `production.env`. A ref deve ser exatamente `oirvvnojgkzdppkdvhej`, e a URL deve ser exatamente `https://oirvvnojgkzdppkdvhej.supabase.co`; formato válido de outra ref também é recusado.
- [x] Auth Site URL configurada e relida como `https://setlivre.com`.
- [x] Redirect allowlist mínima configurada e relida como `https://setlivre.com/auth/callback`; os redirects localhost de `supabase/config.toml` permanecem somente locais.
- [x] JWT expiry `3600`, refresh token rotation ativa/reuse interval `10`, anonymous e telefone desativados, confirmação de e-mail obrigatória, login com e-mail não verificado recusado, troca segura de e-mail, reautenticação para senha, OTP de 6 dígitos/3600 segundos e senha mínima 10 com maiúscula/minúscula/dígito foram verificados pela Management API. Cadastro global permanece desativado até PEND-005.
- [x] Os templates PT-BR versionados de confirmação e recuperação, com seus subjects canônicos, foram aplicados e relidos byte a byte pela Management API sem registrar token/hash em logs.
- [x] SSL enforcement do PostgreSQL foi habilitado e relido pela Management API. O pooler IPv4 em modo sessão recusou uma conexão sem SSL e aceitou `verify-full` com a CA oficial.
...
- [x] Uma senha de 256 bits exclusiva para `app_runtime_prod`, comprovadamente diferente da senha administrativa, foi criada no target protegido `Set Livre:Supabase production app runtime password`. A URL DAL usa o pooler IPv4 em modo sessão, porta `5432`, usuário do projeto e `options=-c role=app_dal`, e está no target `Set Livre:Supabase production database URL app DAL`; nenhum dos valores foi impresso ou versionado.
- [ ] Depois que o configurador criar o diretório privado da VM, copie a URL completa para `/etc/setlivre-deployer/credentials/database-url-app-dal`, arquivo físico `root:root` modo `0600` entregue ao agente por `LoadCredential`.
...
O environment `production` já existe, aceita somente a branch `main` e está com bypass administrativo desativado. Ele não recebe secrets nem executa mutação remota: apenas delimita a publicação do artifact público. A autorização vigente é o ciclo de `@codex review` até parecer limpo, merge protegido e habilitação coordenada das travas independentes no repositório e no host; não existe autoaprovação ou segundo revisor fictício.
...
- [x] O token de deploy fine-grained foi criado para o resource owner `PedroRomeroM`, limitado ao
      repositório `set-livre`, com `Contents: read` e `Actions: read and write`, e guardado sem
      exposição no Gerenciador de Credenciais do Windows, target
      `Set Livre:GitHub production deploy token`. Uma leitura em memória comprovou formato
      `github_pat_...`, repository ID `1328339374`, `Contents: read` e `Actions: read`; o uso de
      `Actions: write` continua restrito ao `workflow_dispatch` canônico de regeneração de artifact
      expirado e só será exercitado quando o workflow exato existir em `main`.
...
- [ ] Antes de habilitar entrega, releia `PRD_PUBLIC_APP_URL`, `PRD_BACKOFFICE_APP_URL`, `PRD_SUPABASE_PROJECT_REF`, `PRD_SUPABASE_URL` e `PRD_SUPABASE_ANON_KEY` nos dois lados. O GitHub já está vinculado ao projeto exato; a VM ainda precisa comprovar igualdade byte a byte. Outra ref de 20 caracteres não autoriza build ou deploy.
...
- [x] O Environment aceita somente `main`, está com `can_admins_bypass=false` e não contém variáveis nem secrets; as entradas públicas do build ficam no nível do repositório.
...
- [x] Bypass administrativo, force-push e deletion estão bloqueados; o Environment aceita apenas `main` e também recusa bypass de administradores. O artifact build-only não usa approval próprio, e a mutação continua condicionada às duas travas independentes e ao protocolo root-owned da VM.
...
- [x] Depois da migração e da execução elevada no Windows, a raiz `%USERPROFILE%\.oci` foi
      normalizada para DACL protegida com exatamente usuário atual, `SYSTEM` e `Administrators`.
      Config, chave e token de sessão passaram novamente pelo contrato do provisionador; a sessão
      `SET_LIVRE` foi renovada no navegador em `2026-08-23`. Ela continua temporária e nunca substitui
      a validação imediatamente anterior ao Plan/Apply.
...
- [ ] Aguarde um Plan canônico com capacidade `AVAILABLE` e reprovisione uma VM limpa. As sondas atuais em `2026-08-23` ainda retornaram `OUT_OF_HOST_CAPACITY`; não usar fallback pago, outra região, outro projeto ou bypass do relatório.
...
- [ ] Releia as variáveis públicas de repositório e confirme igualdade com as APIs GitHub/Supabase antes de habilitar qualquer entrega. Preencha `/etc/setlivre-deployer/production.env`, em sessão administrativa protegida, somente com os valores não secretos aceitos pelo template: `GITHUB_REPOSITORY_ID=1328339374`, os dois workflow IDs positivos e distintos, URLs públicas, ref exata `oirvvnojgkzdppkdvhej`, URL exata `https://oirvvnojgkzdppkdvhej.supabase.co`, chave publicável, `SUPABASE_SERVER_CA_SHA256` e `PRD_DEPLOY_ENABLED=false`. Não adicione token, senha ou URL DAL a esse arquivo.
- [x] O **Server root certificate** foi baixado do painel do projeto, validado como a `Supabase Root 2021 CA` autoassinada e vigente até 2031, e guardado no target protegido `Set Livre:Supabase production server root certificate`. O SHA-256 dos bytes oficiais é `700723581420dd1ac98fd7e9ac529f0ef210eadcaf87fc868a3ad7d114c2f3b7`; grave exatamente esse valor em `SUPABASE_SERVER_CA_SHA256` quando a VM existir.
- [x] O personal access token criado pela autenticação oficial da Supabase CLI comprovou acesso ao projeto exato e foi duplicado, sem exposição, no target protegido `Set Livre:Supabase production access token` para futura instalação do host.
...
- [ ] Mantenha `PRD_DEPLOY_ENABLED=false`. Em `2026-08-23`, o projeto Supabase de São Paulo ficou saudável e publicamente vinculado, mas continua vazio; sondas OCI E2 repetidas encerraram fail-closed com `OUT_OF_HOST_CAPACITY`. VM, hardening, agente/configuração privada do host e migrations Cloud continuam pendentes. PR, merge e deploy não foram executados; raiz/`www` ainda apontavam para parking na última leitura, `ops` não tinha origem e TLS não está comprovado.
