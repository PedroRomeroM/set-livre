-- O readiness aceita ACLs PUBLIC em relações de pg_catalog somente quando
-- elas já pertencem aos privilégios iniciais canônicos registrados pelo
-- PostgreSQL. Isso preserva a leitura pública intencional dos catálogos,
-- inclusive os grants por coluna de pg_subscription, sem permitir que um
-- grant posterior exponha relações restritas como pg_authid.
