-- Estende o containment da ACL pública canônica às rotinas de pg_catalog.
-- pg_init_privs registra ACLs iniciais que diferem do default do objeto; quando
-- não existe registro i/e para uma rotina initdb ou membro real de extensão,
-- acldefault('f', proowner) representa sua baseline inicial implícita. Rotinas
-- normais posteriores não herdam essa exceção. A comparação usa o OID de cada
-- overload e recusa grantor, tipo ou grant option além dessa baseline.
        ...
        and not (
          ...
          or (
            ...
            and exists (
              ...
              where initial_privilege.grantee = current_privilege.grantee
                and initial_privilege.grantor = current_privilege.grantor
                and initial_privilege.privilege_type =
                  current_privilege.privilege_type
                and (
