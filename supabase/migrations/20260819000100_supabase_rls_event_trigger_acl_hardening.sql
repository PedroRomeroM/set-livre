-- set-livre:migration-mode=expand-only

-- O helper é gerenciado pelo Supabase e não existe em todas as imagens.
-- Quando presente, nenhuma role externa ou DAL pode executá-lo. O bloco não
-- cria um substituto local e não mascara divergências da plataforma.
do $managed_rls_acl$
begin
...
-- A release imediatamente anterior continua apta a servir durante rollback.
-- A definição completa preserva o manifesto DAL vigente sem cirurgia dinâmica.
