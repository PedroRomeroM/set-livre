...
create temporary table authorized_dal_routines (
  signature text primary key
) on commit drop;

insert into authorized_dal_routines (signature)
values
...
select ok(
  ...
  'PUBLIC recebe somente CONNECT no banco atual'
);
...
select ok(
  ...
  'app_dal não cria tabelas temporárias por privilégio efetivo'
);
...
select ok(
  ...
  'login local do runtime não cria tabelas temporárias por privilégio efetivo'
);
...
select ok(
  (
    ...
    from pg_catalog.pg_database as database
    join pg_catalog.pg_roles as role on role.rolname = 'dashboard_user'
    cross join lateral pg_catalog.aclexplode(database.datacl) as privilege
    where database.datname = pg_catalog.current_database()
      and privilege.grantee = role.oid
      and privilege.privilege_type = 'TEMPORARY'
  ),
  'grant TEMPORARY explícito gerenciado da dashboard permanece intacto'
);
...
select ok(
  not exists (
    select 1
    from authorized_dal_routines as authorized_routine
    where pg_catalog.to_regprocedure(authorized_routine.signature) is null
  ),
  'entrypoints privados autorizados da DAL existem'
);
...
select ok(
  not exists (
    ...
    where not pg_catalog.has_function_privilege(
      'app_dal',
      authorized_routine.signature,
      'EXECUTE'
    )
  ),
  'app_dal executa os dezenove entrypoints explicitamente autorizados'
);
...
select ok(
  ...
  'PUBLIC não executa entrypoint privado da DAL'
);
...
select ok(
  ...
  'anon não executa entrypoint privado da DAL'
);
...
select ok(
  ...
  'authenticated não executa entrypoint privado da DAL'
);
...
select ok(
  ...
  'service_role não executa entrypoint privado da DAL por padrão'
);
...
select ok(
        ...
        and pg_catalog.bool_and(
          ...
          or (
            ...
            and dependency.objid in (
              select pg_catalog.to_regprocedure(authorized_routine.signature)
              from authorized_dal_routines as authorized_routine
            )
            and dependency.objsubid = 0
          )
  ...
  'app_dal preserva somente as vinte dependências ACL autorizadas, sem grant option'
);
...
select ok(
  not private.check_readiness('20260816000200'),
...
select ok(
  ...
  'grant TEMPORARY a PUBLIC alcança efetivamente DAL e login runtime'
);
...
select ok(
  ...
  'revogação de PUBLIC remove TEMPORARY efetivo de DAL e login runtime'
);
...
select ok(
  has_table_privilege('app_dal', 'pg_catalog.pg_class', 'SELECT')
    and not has_table_privilege('app_dal', 'pg_catalog.pg_authid', 'SELECT')
    and not has_any_column_privilege(
      'app_dal',
      'pg_catalog.pg_authid',
      'SELECT'
    )
    and private.check_readiness('20260816000200'),
  'readiness preserva ACL pública inicial sem expor pg_authid'
);

select ok(
  (
    select routine.oid < 16384
...
-- O runner pgTAP conecta como postgres, enquanto os catálogos da imagem
-- Supabase pertencem a supabase_admin. O dblink existe somente nesta
-- transação de teste e cada drift usa uma transação administrativa local
-- independente, revertida antes de validar a recuperação.
create extension dblink with schema extensions;

do $block$
begin
  perform extensions.dblink_connect(
    'readiness_catalog_acl_probe',
    pg_catalog.format(
...
do $block$
begin
  perform extensions.dblink_exec('readiness_catalog_acl_probe', 'begin');
...
select ok(
  (
    select not probe.canonical_owner
      and probe.public_execute
      and not probe.ready
  ...
  'readiness recusa owner drift na baseline implícita de rotina initdb'
);
...
begin
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    'revoke execute on function pg_catalog.current_database() from public'
  );
end
$block$;
...
select ok(
  (
    select not probe.public_execute and not probe.ready
    from extensions.dblink(
      'readiness_catalog_acl_probe',
      $remote$
        select
  ...
  'readiness recusa owner initdb não canônico mesmo sem EXECUTE público'
);

do $block$
begin
  perform extensions.dblink_exec('readiness_catalog_acl_probe', 'rollback');
end
$block$;
...
begin
  ...
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    $remote$
      create function pg_catalog.readiness_extension_owner_probe()
      returns boolean
      language sql
      stable
      set search_path = ''
  ...
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    'alter extension plpgsql add function pg_catalog.readiness_extension_owner_probe()'
  );
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    $remote$
      delete from pg_catalog.pg_init_privs
      where classoid = 'pg_catalog.pg_proc'::pg_catalog.regclass
        and objoid = pg_catalog.to_regprocedure(
  ...
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    'grant execute on function pg_catalog.readiness_extension_owner_probe() to public'
  );
end
$block$;
...
select ok(
  (
    select probe.later_oid
      and probe.extension_owned
      and probe.no_initial_acl
      and probe.canonical_owner
      and probe.public_execute
      and probe.canonical_grantor
      and probe.ready
    from extensions.dblink(
      'readiness_catalog_acl_probe',
      $remote$
        select
          ...
          exists (
            ...
            where privilege.grantee = 0
              and privilege.grantor = extension.extowner
              and privilege.privilege_type = 'EXECUTE'
              and not privilege.is_grantable
          ),
          private.check_readiness('20260816000200')
      ...
      no_initial_acl boolean,
      canonical_owner boolean,
      public_execute boolean,
      canonical_grantor boolean,
      ready boolean
    )
  ),
  'readiness preserva baseline implícita de membro posterior de extensão'
);
...
begin
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    'alter function pg_catalog.readiness_extension_owner_probe() owner to postgres'
  );
end
$block$;
...
select ok(
  (
    select probe.owner_drift
      and probe.public_execute
      and probe.drifted_grantor
      and not probe.ready
    from extensions.dblink(
      'readiness_catalog_acl_probe',
      $remote$
      ...
      owner_drift boolean,
      public_execute boolean,
      drifted_grantor boolean,
      ready boolean
    )
  ),
  'readiness recusa grantor derivado de owner adulterado em membro de extensão'
);
...
begin
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    'revoke execute on function pg_catalog.readiness_extension_owner_probe() from public'
  );
end
$block$;
...
select ok(
  ...
  'readiness recusa owner de extensão não canônico mesmo sem EXECUTE público'
);
...
select ok(
  (
    select probe.dal_has_select and not probe.ready
    from extensions.dblink(
      'readiness_catalog_acl_probe',
      $remote$
        select
  ...
  'readiness recusa grant PUBLIC efetivo na relação pg_authid'
);
...
begin
  ...
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    'grant select (rolpassword) on table pg_catalog.pg_authid to public'
  );
end
$block$;
...
select ok(
  ...
  'readiness recusa grant PUBLIC efetivo na coluna rolpassword'
);
...
select ok(
  (
    select probe.public_exact_overload
      and not probe.public_other_overload
      and not probe.ready
    from extensions.dblink(
      'readiness_catalog_acl_probe',
      $remote$
      ...
      public_other_overload boolean,
      ready boolean
    )
  ),
  'readiness recusa grant PUBLIC no overload exato pg_read_file(text)'
);
...
begin
  ...
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    $remote$
      create function pg_catalog.readiness_public_routine_probe()
      returns boolean
      language sql
      stable
      set search_path = ''
  ...
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    'grant execute on function pg_catalog.readiness_public_routine_probe() to public'
  );
end
$block$;
...
select ok(
  (
    select probe.public_execute
      and not probe.has_initial_acl
      and not probe.extension_owned
      and not probe.ready
    from extensions.dblink(
      'readiness_catalog_acl_probe',
      $remote$
        select
          ...
          exists (
            ...
            where initial_acl.classoid =
                'pg_catalog.pg_proc'::pg_catalog.regclass
              and initial_acl.objoid = pg_catalog.to_regprocedure(
                'pg_catalog.readiness_public_routine_probe()'
              )
              and initial_acl.objsubid = 0
              and initial_acl.privtype in ('i', 'e')
      ...
      public_execute boolean,
      has_initial_acl boolean,
      extension_owned boolean,
      ready boolean
    )
  ),
  'readiness não trata acldefault de rotina normal posterior como baseline'
);
...
begin
  ...
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    'grant execute on function pg_catalog.abs(integer) to postgres with grant option'
  );
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    'set role postgres'
  ...
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    'grant execute on function pg_catalog.abs(integer) to public'
  );
  perform extensions.dblink_exec(
    'readiness_catalog_acl_probe',
    'reset role'
...
select ok(
  (
    select probe.public_grantors = 2 and not probe.ready
    from extensions.dblink(
      'readiness_catalog_acl_probe',
      $remote$
        select
          (
            select pg_catalog.count(distinct privilege.grantor)
            from pg_catalog.pg_proc as routine
            cross join lateral pg_catalog.aclexplode(
              coalesce(
                routine.proacl,
            ...
            where routine.oid =
                pg_catalog.to_regprocedure('pg_catalog.abs(integer)')
              and privilege.grantee = 0
              and privilege.privilege_type = 'EXECUTE'
          ),
          private.check_readiness('20260816000200')
  ...
  'readiness recusa grant PUBLIC equivalente emitido por grantor não canônico'
);

do $block$
begin
  perform extensions.dblink_exec('readiness_catalog_acl_probe', 'rollback');
  perform extensions.dblink_disconnect('readiness_catalog_acl_probe');
end
$block$;

select ok(
  (
    select pg_catalog.count(distinct privilege.grantor) = 1
    from pg_catalog.pg_proc as routine
    cross join lateral pg_catalog.aclexplode(
      coalesce(
        routine.proacl,
