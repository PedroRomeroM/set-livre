...
function validatedWindowsSystemRoot(systemRoot) {
  ...
  if (!/^[A-Za-z]:\\$/.test(parsedRoot) || normalizedRoot === parsedRoot) {
    throw new Error("O diretório de sistema do Windows não é confiável.");
  }
...
export function terminateWindowsProcessTree(
  pid,
  { runTaskkill = spawnSync, systemRoot = process.env.SystemRoot } = {},
) {
  if (!Number.isSafeInteger(pid) || pid <= 0) {
    throw new Error("O PID da árvore de desenvolvimento é inválido.");
  }
  ...
  if (result.error !== undefined || result.signal !== null || result.status !== 0) {
    throw new Error("A árvore de desenvolvimento não pôde ser encerrada no Windows.");
  }
}
