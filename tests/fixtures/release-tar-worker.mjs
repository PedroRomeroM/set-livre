...
const stickyDirectory = resolve(releaseRoot, "app/sticky");
const regularFile = resolve(releaseRoot, "app/config.json");
const executableFile = resolve(nestedDirectory, "start.sh");
const setgidExecutableFile = resolve(nestedDirectory, "setgid.sh");
const setuidExecutableFile = resolve(nestedDirectory, "setuid.sh");
const archivePath = resolve(workspace, "release.tar.gz");
mkdirSync(nestedDirectory, { recursive: true });
mkdirSync(stickyDirectory);
writeFileSync(regularFile, '{"status":"ready"}\n', "utf8");
writeFileSync(executableFile, "#!/bin/sh\nexit 0\n", { encoding: "utf8", mode: 0o755 });
writeFileSync(setgidExecutableFile, "#!/bin/sh\nexit 0\n", "utf8");
writeFileSync(setuidExecutableFile, "#!/bin/sh\nexit 0\n", "utf8");
chmodSync(setgidExecutableFile, 0o2755);
chmodSync(setuidExecutableFile, 0o4755);
...
process.stdout.write(
  `${JSON.stringify({
    ...
    setgidExecutableMode: lstatSync(setgidExecutableFile).mode & 0o7777,
    setuidExecutableMode: lstatSync(setuidExecutableFile).mode & 0o7777,
    stickyDirectoryMode: lstatSync(stickyDirectory).mode & 0o7777,
  })}\n`,
);
