...
A FEAT-003 é a fatia ativa e ainda não está concluída. A primeira revisão do draft [PR #4](https://github.com/PedroRomeroM/set-livre/pull/4) trouxe três threads: autenticação privada antes do body, fence de mutation A→B e cobertura pgTAP de personas owner/admin sem autoridade antecipada. O snapshot corrigido passou em 563/563 unitários distribuídos por 59 arquivos, 293/293 asserts pgTAP e 91/91 execuções Playwright/axe, incluindo 32/32 da FEAT-003 nos IDs `SL-F003-E2E-001` a `009`. O ID 004 ficou verde nas três projeções com marcador `pagehide=clear`, zero `pageerror`/erro React no console e B inalterado; o ID 009 ficou verde nas quatro projeções de submissão offline, ausência de fila/POST tardio e recuperação. A auditoria terminou sem erros finais, attachments, segredos ou resíduos, com os 62 e-mails QA únicos restritos às 102 ocorrências em títulos automáticos allowlisted (`Fill` 84, `Visible` 10 e `Type` 8). Duas execuções integrais anteriores foram interrompidas em 46 aprovados e serviram somente ao diagnóstico: a primeira revelou a captura tardia do body `409` após o hard reload; na segunda, o oráculo baseado em `document.body.textContent` incluiu scripts Flight/RSC com o snapshot SSR de A, fora da superfície visual, e não distinguiu esses bytes de um nó privado ainda renderizado. Esse match não comprovou vazamento visual. A captura da resposta foi antecipada, o boundary passou a fechar por commit React síncrono seguro e o novo probe varre somente `main`, exigindo a desconexão efetiva de heading, resumo, formulário e controles; a matriz corrente comprovou esse contrato. Os builds e smokes standalone dos dois apps também passaram sem warnings. O snapshot funcional foi congelado no commit `f4f3b1d13238bdb67a2bc77bff55c119132040dc`, que gerou a release local canônica com 2.809 artefatos; o archive `set-livre-f4f3b1d13238bdb67a2bc77bff55c119132040dc.tar.gz` possui SHA-256 `571a0dbdee91d17c47158e0b00aaa0c6bcd4ce6d2f4ffa7f06f1fb6afc4ff887`. O commit documental `9c23ef3f0818d60a84b4f6321905c389cff5b7fa` é o HEAD remoto revisado. O trigger `5261407908` foi publicado às `02:29:08Z`; após a espera integral de 60 minutos, a inspeção confirmou a resposta Codex `5261469771`, publicada às `02:35:40Z` sobre `9c23ef3f08`, “Didn't find any major issues”. As três threads estão resolvidas e a leitura thread-aware registra zero aberta. O PR permanece `OPEN`/draft e somente o merge está pendente; nada da FEAT-004 deve ser antecipado. A ordem não deve ser duplicada neste contexto; mudanças de sequência exigem atualização do ADR-017, da ordem especializada e da rastreabilidade.
