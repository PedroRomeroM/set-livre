...
- O snapshot Windows atual passou por `npm ci`, format, lint, typecheck, 991 unitários com 27 skips condicionais de plataforma, docs:check 34/200/23, audit sem vulnerabilidades, Knip e diff-check. A matriz Playwright integral passou em 114/114 por 17 specs/16 projetos, sem retry, e os builds standalone web/backoffice concluíram 26 + 4 rotas. CI hospedado, release Linux x86_64, Supabase São Paulo, VM e deploy continuam exigindo evidência própria.
...
O próximo passo é concluir a validação integral do delta Windows/infraestrutura, criar o Supabase de produção em São Paulo e a VM Oracle E2.1.Micro somente após os gates de custo/capacidade, aplicar hardening e smoke, fazer commit, executar a auditoria leve pós-commit e então abrir o PR para os ciclos de `@codex review`. Produção continua bloqueada até o merge e os dois deploys reais terminarem verdes.
