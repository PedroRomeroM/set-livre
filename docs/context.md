...
- A FEAT-004 está em implementação na branch própria, com o [PR #6](https://github.com/PedroRomeroM/set-livre/pull/6). As correções dos dois P2 do primeiro review separam ativação completa (21 colunas com documento) de recebimentos compactos (16 sem título, versão textual, hash ou corpo) e exigem GET autoritativo sem replay para conflito/validação sem campo; ambas as threads receberam resposta e foram resolvidas. A branch avançou no remoto de `3e3f866c42302df9b0499e9af75575c7c092f3f0` até `011a48f4910baa0e17b26dee6eda3c678d910572`: o commit funcional é `440c81f6cc44cc95ed281d84e9a5124ae98a59c4`, e `011a48f4...` registra a documentação da release. No snapshot confirmado, HEAD local e remoto coincidiam em `011a48f4...`. O segundo review `PRR_kwDOTyzZrs8AAAABJV08Cw`, submetido em `2026-08-12T22:59:35Z` sobre `3e3f866c...`, abriu `PRRT_kwDOTyzZrs6YwM7k`: `owner_contract_not_current` era agrupado com outros `42501` e retornava `403`. O patch publicado traduz somente `42501 + owner_contract_not_current` para `409 CONFLICT`, preserva bloqueios e demais `42501` como `403 FORBIDDEN` e reutiliza o boundary verification-first já existente. A thread recebeu [`PRRC_kwDOTyzZrs7h4a21`](https://github.com/PedroRomeroM/set-livre/pull/6#discussion_r3789663669), REST `3789663669`, em `2026-08-15T15:36:08Z`; depois da verificação na própria thread, `PedroRomeroM` a resolveu ainda atual e não outdated. A evidência atual está integralmente verde: Node 24 em 718/718 unitários de 74 arquivos e todos os gates estáticos; reset/banco em 355/355; Playwright focado único 23/23 e integral único 114/114, com privacidade/cleanup verdes; build único sem warnings; smoke real padrão mais FEAT-004 com 14/11 nonces, três boundaries, dois redirects, banco `0 → 0` e cleanup zero; e release canônica local do commit funcional `440c81f6...`. A release `79376b62...` permanece histórica. No snapshot imediato, o PR #6 seguia `OPEN`/draft contra `main@174ee16342367caedf55521227d21d5bf076b1a9`; `statusCheckRollup=[]` e `mergeable`/`mergeStateStatus=UNKNOWN` não são evidência de checks nem mergeabilidade. Novo `@codex review`, espera integral de 60 minutos, promoção e merge ainda faltam. A prova x64 também não é ARM64 ou produção; gateway externo, checkout, fallback administrativo, papel admin, dados bancários, PEND-003/004/006 e o smoke ARM64 nativo continuam fora ou bloqueando produção.
...
Concluir o ciclo de revisão do novo P2 do [PR #6](https://github.com/PedroRomeroM/set-livre/pull/6): gates integrais, banco, browser, build, smoke, release canônica local `440c81f6...`, publicação até `011a48f4...` e resposta/resolução de `PRRT_kwDOTyzZrs6YwM7k` já estão concluídos. Agora é preciso solicitar novo `@codex review`, respeitar a espera integral de 60 minutos e, somente se a inspeção for limpa, promover para ready e fazer merge. As duas threads anteriores também estão respondidas e resolvidas. A release `79376b62...` é fotografia histórica anterior ao delta. PEND-003 e o smoke ARM64 nativo continuam bloqueando produção. Mudanças de sequência exigem atualização do ADR-017, da ordem especializada e da rastreabilidade.
