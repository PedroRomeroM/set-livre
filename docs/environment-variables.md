...
| repository variable | `PRD_SUPABASE_PROJECT_REF` | ref pública exata `oirvvnojgkzdppkdvhej`; outra ref é proibida          |
| repository variable | `PRD_SUPABASE_URL`         | URL exata `https://oirvvnojgkzdppkdvhej.supabase.co`                    |
| repository variable | `PRD_SUPABASE_ANON_KEY`    | publishable key moderna desse projeto; nunca autoridade ou secret       |
...
| `PRD_SUPABASE_PROJECT_REF`  | ref pública exata `oirvvnojgkzdppkdvhej`                             |
| `PRD_SUPABASE_URL`          | URL pública exata `https://oirvvnojgkzdppkdvhej.supabase.co`         |
...
O projeto produtivo está saudável em `sa-east-1`, mas permanece vazio, sem branches e sem migrations. As repository variables foram atualizadas e `PRD_DEPLOY_ENABLED=false` foi preservado; a VM ainda precisa receber e provar os mesmos valores. Em `2026-08-23`, não havia VM Set Livre ativa e sondas E2 repetidas encerraram fail-closed com `OUT_OF_HOST_CAPACITY`; agente/configuração do host e migrations Cloud não estavam aplicados, o domínio raiz/`www` ainda apontava para parking, `ops` não tinha origem e TLS não estava comprovado. PR, merge e deploy não foram executados. A presença dos nomes acima não constitui gate, configuração válida ou deploy.
