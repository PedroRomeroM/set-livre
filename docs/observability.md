...
- `current_user=app_dal` sem atributo privilegiado, membership de saída, ownership ou grant direto além de `USAGE private` e `EXECUTE` nas doze rotinas autorizadas: dois checks de readiness, criação da intenção legal, seis operações do contexto recovery e três comandos do perfil, sempre sem grant option; o manifesto totaliza treze dependências ACL exatas. A leitura do perfil fica fora dessa role e usa RPC público `security invoker` com `auth.uid()` + RLS;
