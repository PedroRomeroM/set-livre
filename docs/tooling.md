...
| ------------------ | ------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| desenvolvimento    | `npm run dev`, `dev:backoffice`, `dev:all`                                            | portas 3000/3001; `dev:all` valida env local, isola os filhos e encaminha sinais                                     |
...
`dev:all` faz preflight dos dois `.env.local` antes do primeiro spawn. O launcher exige arquivo físico `0600`, os seis nomes runtime exatos, origens web/backoffice corretas, Supabase HTTP em loopback `:54321` e a identidade DAL local restrita. Cada filho recebe seu próprio arquivo e uma allowlist operacional mínima; valores cloud, banco, E2E, loader, Git, npm ou SSH exportados pelo host não têm precedência. O validador puro da URL DAL é compartilhado com o smoke de release para impedir divergência entre os dois caminhos locais.
...
Os workers em `tests/fixtures/` são entradas explícitas do Knip porque são executados como processos Node reais, sem import estático. O binário de sistema `flock`, versionado e documentado na matriz de dependências, é a única exceção de binário externo da análise.
