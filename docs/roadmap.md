...
existe enquanto a feature está planejada ou sendo implementada e é removido assim que código, testes e
documentação permanente comprovam o recorte. O comportamento concluído fica no código, nos testes e
nos documentos permanentes de domínio; review, merge e deploy continuam rastreados no PR e nos checks.
...
fatia ainda está incompleta, o estado é `Em andamento`. Depois que implementação, testes e contratos
permanentes estão completos, o plano é removido e a linha vira `Concluída`; isso não substitui o ciclo
obrigatório de review, merge e deploy do PR.
