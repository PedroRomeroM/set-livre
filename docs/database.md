...
A migration head atual é `20260810000200`. A fundação não antecipa nenhuma tabela de domínio e aplica somente:
...
- a baseline pública permite exatamente `USAGE` em `pg_catalog`/`information_schema`, `CONNECT`/`TEMPORARY` no banco e `USAGE` nas quatro linguagens internas, sem grant option. Ela recusa ACL pública em outros schemas, defaults, objetos grandes, parâmetros, FDW/servers e tablespaces. Em toda relação ou coluna de `pg_catalog`, um privilégio efetivo de `PUBLIC` precisa já existir nos privilégios iniciais `i`/`e` de `pg_init_privs`; privilégios por coluna podem ser cobertos pelo grant inicial da própria coluna ou da relação. Assim, leituras built-in legítimas continuam disponíveis, enquanto grants posteriores em catálogos restritos como `pg_authid` falham fechado. `pg_roles`, `pg_user` e `pg_db_role_setting` conservam ainda a baseline mais estrita de somente owner administrativo e `SELECT` de `postgres`, sem ACL por coluna; roles web/DAL também não podem alcançar esses três catálogos por membership transitiva. O contrato não afirma que todo catálogo built-in seja confidencial. Enquanto o ADR-018 suspender APIs externas, `app_dal`, `anon`, `authenticated`, `service_role` e `PUBLIC` não usam nem leem/escrevem/executam o schema `net`; o worker administrativo local conserva apenas o acesso necessário;
- 134 asserts pgTAP, incluindo funções-probe que comprovam deny-by-default nos três schemas, ACLs efetivas, o estado exato das roles e a detecção/restauração de grants, ownership, parâmetros, atributos ou memberships adulterados;
