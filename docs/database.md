...
A migration head atual é `20260810000300`. A fundação não antecipa nenhuma tabela de domínio e aplica somente:
...
- readiness consulta as duas funções privadas por um subpath compartilhado `server-only` e falha se o login ou a role efetiva `app_dal` possuir login, herança, criação, replicação, superuser ou `BYPASSRLS`; `app_dal` também deve permanecer sem qualquer membership de saída, pois `NOINHERIT` não impede `SET ROLE`;
...
- a baseline pública permite exatamente `USAGE` em `pg_catalog`/`information_schema`, `CONNECT`/`TEMPORARY` no banco e `USAGE` nas quatro linguagens internas, sem grant option. Ela recusa ACL pública em outros schemas, defaults, objetos grandes, parâmetros, FDW/servers e tablespaces. Em toda relação ou coluna de `pg_catalog`, um privilégio efetivo de `PUBLIC` precisa já existir nos privilégios iniciais `i`/`e` de `pg_init_privs`; privilégios por coluna podem ser cobertos pelo grant inicial da própria coluna ou da relação. Nas rotinas desse schema, cada entrada pública atual precisa estar contida, com grantor e grant option, na ACL `i`/`e` do OID exato. Se uma rotina initdb (`OID < 16384`) ou membro real de extensão não possui registro inicial, `acldefault('f', proowner)` representa somente sua baseline implícita; uma rotina normal posterior sem `pg_init_privs` tem baseline pública vazia. Isso preserva, por exemplo, o `EXECUTE` built-in de `current_database()` sem aceitar um grant em `pg_read_file(text)` nem uma função normal nova. `pg_roles`, `pg_user` e `pg_db_role_setting` conservam ainda a baseline mais estrita de somente owner administrativo e `SELECT` de `postgres`, sem ACL por coluna; roles web/DAL também não podem alcançar esses três catálogos por membership transitiva. O contrato não classifica a segurança semântica de cada rotina, não cobre por si só grants a roles nomeadas e não afirma que todo catálogo built-in seja confidencial. Enquanto o ADR-018 suspender APIs externas, `app_dal`, `anon`, `authenticated`, `service_role` e `PUBLIC` não usam nem leem/escrevem/executam o schema `net`; o worker administrativo local conserva apenas o acesso necessário;
- 141 asserts pgTAP, incluindo funções-probe que comprovam deny-by-default nos três schemas, ACLs efetivas, o estado exato das roles e a detecção/restauração de grants, ownership, parâmetros, atributos ou memberships adulterados;
