...
### 1.1 Estado implementado em `main` e hardening da stack local

A árvore possui 16 migrations append-only e head `20260819000100`. No ambiente Windows nativo, a cadeia canônica `supabase:reset` → `supabase:generate` → `test:db` terminou com sucesso nesse head; as quatro suítes pgTAP somam 361/361 asserts (`161 + 78 + 57 + 65`) e os artefatos gerados conferem byte a byte com a instância local. A fundação continua intacta; a FEAT-002 fornece identidade mínima, `legal-core` e recovery, a FEAT-003 completa o perfil e a FEAT-004 acrescenta a autoridade de dono e o recebedor local:
...
- `20260812000200_owner_recipient_projection_split.sql`: correção append-only do review que preserva a migration anterior, renomeia a projeção completa de 21 colunas para `public.get_owner_activation_status()` e recria `public.get_owner_recipient_status()` com 16 colunas, sem título, versão textual, hash ou corpo Markdown do contrato; grants continuam apenas para `authenticated` e o manifesto privado da DAL não cresce.
- `20260815000100_owner_audit_request_correlation.sql`: separa correlação e replay em `audit.events`, acrescenta `idempotency_key NOT NULL`, troca a unicidade para `(action, target_id, idempotency_key)` e substitui os overloads de ativação/aplicação por assinaturas que recebem `requestId` e chave idempotente separadamente. Linhas legadas copiam o valor antigo de `request_id` para `idempotency_key`; o `request_id` legado não é reescrito porque a correlação HTTP histórica verdadeira não pode ser recuperada.
- `20260819000100_supabase_rls_event_trigger_acl_hardening.sql`: correção append-only para o contrato atual do Supabase/PostgreSQL. Ela substitui integralmente `private.check_readiness(text)` por uma definição autoritativa, preserva o manifesto exato de 17 dependências ACL e 16 rotinas DAL, exige o head atual ou o predecessor imediato durante a aplicação e revoga `EXECUTE` público do helper gerenciado `public.rls_auto_enable()` quando presente. O snapshot SQL é produzido somente pelo gerador canônico; não existe edição manual do artefato.
- login `app_runtime_local` criado e rotacionado fora das migrations pelo bootstrap local, com atributos, memberships, parâmetros, ownership e grants diretos reconciliados antes de assumir `app_dal` explicitamente. Seu manifesto permite somente `CONNECT` direto no banco atual, a membership de saída para `app_dal`, a referência administrativa de `postgres` sem `SET/INHERIT` e uma máscara vazia para o GUC local de assinatura JWT; qualquer ACL, ownership, parâmetro ou membro adicional falha fechado. O mesmo bootstrap aceita `pg_net` ausente enquanto APIs externas estiverem suspensas; se a extensão existir, usa somente o superuser da stack local para fechar schema, tabelas, sequências, funções e defaults de `net`, preservando exclusivamente os privilégios administrativos exigidos pelo worker sob `postgres`;
