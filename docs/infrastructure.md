...
- Docker Desktop `4.87.0` saudável com containers Linux pelo backend Hyper-V, contexto `desktop-linux`, endpoint `npipe:////./pipe/dockerDesktopLinuxEngine`, engine `29.7.2` e `OSType=linux`;
...
O discovery inicial comprovou `sa-saopaulo-1` como home region e uma AD; as tentativas A1/ARM64 daquela fase histórica retornaram `OUT_OF_HOST_CAPACITY`. Após aprovação humana registrada no ADR-021, uma instância diagnóstica `set-livre-production` foi comprovada `RUNNING` em `2026-08-18` com `VM.Standard.E2.1.Micro`, Ubuntu 24.04 x86_64, boot 50 GB e IMDSv2-only. O SSH administrativo por chave foi confirmado. Ao revelar uma falha real no bootstrap anterior, a instância e seu boot volume foram removidos para não conservar host parcialmente configurado. Em `2026-08-23`, a sessão OCI, ACLs, inventário e transição privada de tags v1→v2 foram revalidados; sondas E2 não persistentes repetidas ainda encerraram fail-closed com `OUT_OF_HOST_CAPACITY`. A tenancy admite duas E2 Micro Always Free e outra existe fora do projeto; a posição Set Livre está logicamente liberada, mas sem capacidade física observada. Isso não comprova hardening, runtime ou deploy.
...
As fontes locais dos secrets correspondentes a `GITHUB_DEPLOY_TOKEN`, `SUPABASE_ACCESS_TOKEN`,
`SUPABASE_DB_PASSWORD` e `PRD_DATABASE_URL_APP_DAL`, mais o Server root certificate do projeto
produtivo, já estão protegidas no Gerenciador de Credenciais do Windows. O PAT GitHub fine-grained
comprovou a identidade exata do repositório, `Contents: read` e `Actions: read` sem impressão do
valor; a permissão `Actions: write` permanece reservada ao dispatch canônico descrito abaixo. No host,
esses valores existirão somente como os cinco arquivos root-only de
`/etc/setlivre-deployer/credentials`; não entram em `production.env`. A unit usa `LoadCredential` e o
agente os lê pelo diretório privado `$CREDENTIALS_DIRECTORY`; runtime entra depois em arquivos
root-owned por release. O certificado precisa ser PEM válido, vigente e corresponder ao SHA-256
público fixado; as conexões usam `verify-full`, sem `rejectUnauthorized=false`. O token GitHub é
exclusivo do repositório, com `Contents: read` e `Actions: read and write`; a única mutação autorizada
é o `workflow_dispatch` validado que regenera um artefato expirado de um run de produção previamente
aprovado. O contrato de `production.env` exige repository/workflow IDs exatos e somente configuração
não secreta, incluindo a ref e URL públicas fixadas do projeto `oirvvnojgkzdppkdvhej`; outra ref
válida também falha antes de migration. Os workflow IDs ainda dependem do registro no GitHub. A flag
de repositório e a trava do host permanecem `PRD_DEPLOY_ENABLED=false` até que esses IDs sejam
preenchidos/verificados como positivos e distintos e a sandbox `/run/lock` seja comprovada. O projeto
produtivo de São Paulo está saudável, vazio e sem branches; as repository variables já estão
corretas, mas a configuração privada do host e as migrations Cloud continuam pendentes. Em
`2026-08-23`, não havia VM Set Livre ativa e sondas E2 repetidas encerraram fail-closed com
`OUT_OF_HOST_CAPACITY`. Hardening, agente/configuração do host, raiz/`www`, `ops` e TLS não estão
comprovados. O Environment build-only recebe somente `main`, sem secrets ou mutação; com um único
colaborador, o projeto rejeita autoaprovação teatral e exige os ciclos de `@codex review`, merge
protegido e travas independentes do host. Portanto nenhum CI/deploy de produção é apresentado como
executado ou verde. Os passos humanos canônicos estão em `configuration-seteps.md`.
...
A VM free tier é baseline, não promessa de escala. `VM.Standard.E2.1.Micro` fornece somente 1 OCPU e aproximadamente 1 GB; quando ativa, a instância Set Livre ocupa a segunda e última posição E2 Micro Always Free da tenancy. Em `2026-08-23`, a posição lógica foi revalidada como liberada, mas sondas exatas repetidas reportaram `OUT_OF_HOST_CAPACITY`; nova capacidade precisa ser comprovada antes de agir.
...
O projeto `Set Livre Production`, ref pública `oirvvnojgkzdppkdvhej`, foi criado em `sa-east-1` após aprovação de US$ 10/mês e comprovado saudável em 2026-08-23. O projeto canadense anterior foi apagado sem migrations. O novo projeto iniciou com zero migrations, zero tabelas próprias e zero branches; Security Advisor sem findings e somente o INFO de performance `auth_db_connections_absolute`. Auth foi configurado e relido em 22 invariantes com domínio/callback produtivos, templates PT-BR, senha mínima 10 e cadastro bloqueado até SMTP; a senha administrativa do banco foi rotacionada com 256 bits aleatórios e guardada somente no Gerenciador de Credenciais do Windows. SSL enforcement está ativo: o pooler IPv4 em modo sessão recusou plaintext e aceitou `verify-full` com o certificado oficial. A senha distinta de `app_runtime_prod` e sua URL DAL com `options=-c role=app_dal` também estão protegidas localmente; a role, ACLs gerenciadas, backups e instalação das credenciais no host ainda precisam ser comprovados. A cadeia local atual possui 16 migrations, predecessor `20260815000100` e head `20260819000100`; o pgTAP passou em 4 arquivos e 361/361 testes, e o readiness confere 17 dependências ACL e 16 rotinas DAL. `supabase/config.toml` contém portas/URLs locais e nunca é enviado por `config push`; produção recebe somente migrations versionadas após merge protegido em `main`. Supabase Branching não integra esta fase e testes destrutivos permanecem exclusivamente locais.
