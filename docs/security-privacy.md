...
A matriz 114/114, o build/smoke e a release local `2a86acc4...` permanecem uma fotografia histórica do terceiro P2. No delta de capability, o fechamento estático passou com 734/734 unitários em 74 arquivos; o banco passou em 358/358, e as matrizes browser focada/integral passaram em 23/23 e 114/114. As auditorias finais encontraram zero secret, token, cookie Auth, documento, referência privada de provider ou telefone; os e-mails QA ficaram apenas em títulos allowlisted, e banco, Mailpit, portas e processos terminaram em zero. A primeira auditoria integral parou por uma variável inexistente do próprio harness depois de estrutura/privacidade; a correção reaproveitou o mesmo relatório sem executar Playwright novamente. Um único build passou, mas o smoke customizado não produziu evidência verde após três rejeições do harness. A release canônica local `969f30cd...` foi gerada exatamente uma vez; smoke embutido, varredura de secrets, paridade e cleanup ficaram verdes, e duas auditorias terminaram `NO-BLOCKER`. Publicação, body, resposta e resolução foram concluídos; novo review permanece pendente, e nenhuma prova x64 substitui ARM64.
...
Na release histórica do segundo P2, o gerador foi invocado uma única vez para `440c81f6...`; tar, smoke, guardas de segurança, secrets/PII e cleanup ficaram verdes. Ela não contém `20260815000100` nem valida o patch atual; tampouco substitui PEND-003 ou o smoke ARM64 nativo.
