...
- o adapter local determinístico implementa somente start/refresh e é recusado fora de `local | test`. A leitura autenticada do contrato `source=local_fixture` permanece disponível para consulta fora desses ambientes, mas a ativação falha fechada. `ownerActivationCapability` é obrigatória somente na projeção completa: fonte `approved` produz `available`; `local_fixture` produz `available` apenas em `local | test` e `unavailable` em `development | production`, ambiente ausente ou inválido. O comando indisponível retorna `503 SERVICE_UNAVAILABLE` antes da escrita; uma futura versão aprovada continua dependente de PEND-006. Pagar.me/Asaas, SDK, HTTP, credenciais, webhook e sandbox remoto continuam suspensos pelo ADR-018 e bloqueados por PEND-004;
- `recipientOnboardingCapability` é uma capacidade efêmera derivada no servidor, não um fato do recebedor nem um alias de `providerMode` ou `nextAction`. Ela é obrigatória nas duas projeções e nos três retornos de comando: `local | test` produz `local_adapter`; `development | production`, ambiente ausente ou valor inválido produzem `unavailable`. O navegador não fornece nem decide esse campo;
- quando a capacidade é `unavailable`, a leitura preserva o estado factual para consulta, mas `recipient.onboarding.start | refresh` retornam `503 PAYMENT_PROVIDER_UNAVAILABLE` antes de `prepare`, sem reservar operação, chamar adapter ou alterar o fato canônico;
