...
- `CONFLICT` e `VALIDATION_FAILED` sem `fieldErrors` desabilitam a ação até um GET autoritativo explícito e nunca repetem o POST. `VALIDATION_FAILED` com erro de campo continua no formulário e pode ser corrigido sem forçar releitura;
- a matriz pós-review aceita provou o fence em concorrência real: uma única tentativa stale, `409`, zero releitura antes da decisão explícita, exatamente um GET autoritativo e nenhum replay. A rodada focada passou em 23/23 e a integral em 114/114; os relatórios anteriores ao review permanecem históricos;
- o smoke runtime pós-review confirmou que as duas APIs privadas de leitura permanecem fechadas ao guest com `401`/UUID e que `/dono` e `/dono/recebimentos` preservam os redirects streaming exatos antes de qualquer hidratação ou cache privado;
